...
export function DecorationStarGraph(
  ...
  this._updateNeighborDecoration = function (markSet, d, duration, curve) {
    ...
    markSet.animateDecoration(
      d,
      "alpha",
      new Animator(interpolateTo(d.data("alpha")), duration, curve)
    );
  };
...
export class WideningLineDecoration extends Decoration {
  ...
  constructor(mark, dataItem, frame, lineWidthFn, lineAlphaFn) {
    super("wideningLine", [mark], {
      ...
      lineWidth: {
        valueFn: () => lineWidthFn(dataItem),
      },
      alpha: {
        valueFn: () => lineAlphaFn(dataItem),
      },
      color: {
  ...
  _getX() {
    return getWithFallback(this.dataItem.xs, this.frame1, this.mark.attr("x"));
  }

  _getY() {
    return getWithFallback(this.dataItem.ys, this.frame1, this.mark.attr("y"));
  }

  _getX2() {
    return getWithFallback(this.dataItem.xs, this.frame2, this.mark.attr("x"));
  }

  _getY2() {
    return getWithFallback(this.dataItem.ys, this.frame2, this.mark.attr("y"));
  }
